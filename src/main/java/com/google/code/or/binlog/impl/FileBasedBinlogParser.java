 ...
 * @author Jingqi Xu
 */
public class FileBasedBinlogParser extends AbstractBinlogParser implements BinlogEventListener {
	//
	...
	public void parse(XInputStream is) throws IOException {
		...
		while(isRunning() && is.hasMore()) {
			try {
				...
				is.setReadLimit((int)(header.getEventLength() - header.getHeaderLength())); // Enforce the event boundary
